...
public class ServiceFirm extends Firm{
	...
	ServiceFirm(ContinuousSpace<Object> space, Grid<Object> grid, int ID, ServiceSector ownSector, int bankID, int clearingHouseID, double initPaymentAccount){
...
//		
		try {
            File file = new File("computePlannedOutputLOG.txt");
            ...
            System.out.println("computePlannedOutput called at tick: " + tick);
		...
		this.plannedOutputProduction = 0;
		...
		for(int i=0; i < mallInventoryList.size();i++) {
			...
			if(this.tick<2) {
				...
				try {
		            File file = new File("primaryDemand.txt");
		            FileOutputStream fos = new FileOutputStream(file, true);		            	            
		            PrintStream ps = new PrintStream(fos);      // Create a PrintStream that writes to the file          
		            System.setOut(ps);                          // Redirect System.out to the PrintStream	            
		            System.out.println("primaryDemand " + aInventory.primaryDemand+ "Initial output  "+initialOutput+"   CURRENT tick: " + tick);
		            ps.close();   // Close the streams
		            fos.close();
		        } catch (Exception e) {
		            e.printStackTrace();
...
//					aInventory.plannedDelivery = Math.max(0.0,  aInventory.refillLevel);
					
					this.plannedOutputBeforeSmoothing += aInventory.plannedDelivery;
					
					try {
			            File file = new File("plannedOutputBeforeSmoothing.txt");
			            FileOutputStream fos = new FileOutputStream(file, true);		            	            
			            PrintStream ps = new PrintStream(fos);      // Create a PrintStream that writes to the file          
			            System.setOut(ps);                          // Redirect System.out to the PrintStream	            
			            System.out.println( "refillLevel  "+aInventory.refillLevel +"plannedOutputBeforeSmoothing " + this.plannedOutputBeforeSmoothing + "   CURRENT tick: " + tick);
			            ps.close();   // Close the streams
			            fos.close();
			        } catch (Exception e) {
			            e.printStackTrace();
		...
		System.out.println("Tick:" + tick + "plannedOutput"+plannedOutput);
...
//		if(this.ownSector.sectorID==10) {	
		...
		try {
            File file = new File("outputServiceFirm.txt");
            FileOutputStream fos = new FileOutputStream(file, true);		            	            
            PrintStream ps = new PrintStream(fos);      // Create a PrintStream that writes to the file          
            System.setOut(ps);                          // Redirect System.out to the PrintStream	            
            System.out.println(
            		"Tick:" + currentTick +  "   Firm " + this.ID + "  Sector ID: " + this.ownSector.sectorID + "    Output: "+this.output+"	Planned output: "+this.plannedOutput+
            		" avg. Market Share: "+this.avgMarketShare+"avg. Unit Costs:"+this.avgUnitCosts+
...
//			System.out.println("Tick:" + currentTick + "   outputCapital:	 " + outputCapital + " outputIntermediate:	"+ outputIntermediate + "	outputLabor: " + outputLabor + "	outputIntermediate:	" + outputIntermediate + "	outputService:	" + outputService + "	outputEnergy:	"+outputEnergy);
            fos.close();
        } catch (Exception e) {
            e.printStackTrace();
        }		
